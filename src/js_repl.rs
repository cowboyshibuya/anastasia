...
impl HelperConnection {
    fn start(deadline: Option<Instant>) -> anyhow::Result<Self> {
        let command = std::env::var_os("ANASTASIA_COMPUTER_USE_SERVER")
            .map(PathBuf::from)
            .ok_or_else(|| {
                anyhow!("ANASTASIA_COMPUTER_USE_SERVER is required before the first sky operation")
            })?;
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    for line in BufReader::new(stderr).lines().map_while(Result::ok) {
                        eprintln!("Anastasia Computer Use: {line}");
                    }
                })?;
        }
