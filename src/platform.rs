...
#[cfg(target_os = "linux")]
pub fn init_reduce_motion(cx: &mut gpui::App) {
    if let Ok(value) = std::env::var("ANASTASIA_REDUCE_MOTION")
...
/// delegate (and therefore click responses); Anastasia only supplies content here
...
/// Keep Anastasia's single main window alive when the user closes it. This preserves
