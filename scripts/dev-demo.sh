...
#
# Everything lives under /tmp/anastasia-demo-*; re-runs reuse it. Ctrl-C cleans up.
set -euo pipefail
cd "$(dirname "$0")/.."

DAEMON_DIR=/tmp/anastasia-demo-daemon
UI_DIR=/tmp/anastasia-demo-ui
IPC=27921
DELAY=""
[[ "${1:-}" == "--slow" ]] && DELAY=350

echo "▸ building (first run takes a few minutes)…"
cargo build -p anastasia -q

echo "▸ starting engine daemon on :$IPC"
env ANASTASIA_DATA_DIR="$DAEMON_DIR" ANASTASIA_IPC_PORT=$IPC ANASTASIA_HARNESS=mock \
  ${DELAY:+ANASTASIA_MOCK_DELAY_MS=$DELAY} RUST_LOG=warn \
  ./target/debug/anastasia headless &
DAEMON_PID=$!
...
probe() { cargo run -q -p anastasia-rpc --example rpc_probe -- "ws://127.0.0.1:$IPC" "$@"; }
...
if [[ ! -f "$DAEMON_DIR/.demo-seeded" ]]; then
  ...
  declare -A SPACES=()
  for project in anastasia soccertcg anastasia aether; do
  ...
  seed "Native Anastasia Rust Rewrite"    anastasia anastasia/main                 0  run
  seed "Rebalance Player Stats Caps"  soccertcg    anastasia/rebalance-player-stat-caps  2  run
  seed "Craft Premium TCG Experience" soccertcg    anastasia/craft-premium-tcg-exp       26 skip
  seed "Initial Context Exploration"  anastasia        anastasia/initial-context-exploration 14 skip
  seed "Soccer TCG Repo Creation"     aether       aether/main                       48 skip
  touch "$DAEMON_DIR/.demo-seeded"
fi

echo "▸ opening anastasia (composer is live — type into it; --slow shows streaming)"
ANASTASIA_DATA_DIR="$UI_DIR" ANASTASIA_IPC_PORT=$IPC RUST_LOG=warn ./target/debug/anastasia
