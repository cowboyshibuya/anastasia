...
//!
//! In anastasia these were synced Postgres rows; in anastasia they live in the per-org
...
pub struct Device {
    ...
    /// First registration time (anastasia devices.created_at — the Devices page
...
pub struct Chat {
    ...
    /// continuity across engine restarts (anastasia's `chats.harness_session_id`).
...
pub struct Worktree {
    ...
    pub path: String,
    pub branch: String,
    /// Generated worktree folder name (`anastasia/<name>` is its branch).
    #[serde(default)]
    pub name: String,
