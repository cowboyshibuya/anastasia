...
/// The app gate (anastasia's App.tsx phases). Pure.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum GatePhase {
...
/// port of anastasia's `formatTimeAgo`.
...
/// Session-row sub-line, "project · branch" (anastasia `chatLocation`): the repo
...
/// Per-kind chip label + one-line detail. Labels match anastasia's `describeTool`
...
pub fn tool_group_summary(tools: &[(crate::ToolCall, bool)]) -> String {
    ...
    // Capitalize the first segment only (anastasia's style).
...
/// read the same on every surface. `anastasia-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (the engine mints a `anastasia/<name>`
