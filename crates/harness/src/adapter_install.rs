...
//! ONCE into a anastasia-owned prefix (`~/.anastasia/adapters/<pkg>/<version>`, own
...
const OK_MARKER: &str = ".anastasia-install-ok";
const INSTALL_TIMEOUT: Duration = Duration::from_secs(600);

/// `$ANASTASIA_ADAPTERS_DIR`, else `~/.anastasia/adapters`.
fn adapters_root() -> Option<PathBuf> {
    if let Some(dir) = std::env::var_os("ANASTASIA_ADAPTERS_DIR").filter(|d| !d.is_empty()) {
        return Some(PathBuf::from(dir));
    }
    std::env::var_os("HOME")
        .filter(|h| !h.is_empty())
        .map(|h| PathBuf::from(h).join(".anastasia").join("adapters"))
}
