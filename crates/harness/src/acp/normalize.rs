...
use anastasia_proto::{AgentEvent, SlashCommand, TodoItem, ToolCall, ToolDiff};
...
/// content) to the typed [`ToolCall`] anastasia renders. Best-effort: agents vary
...
pub(crate) fn map_update(update: &Value) -> Vec<AgentEvent> {
    ...
    match kind {
        ...
        "plan" => {
            ...
            vec![
                AgentEvent::ToolCall {
                    id: anastasia_proto::LIVE_PLAN_TOOL_ID.into(),
        ...
        // Context-window gauge, not per-turn input/output tokens — anastasia's
