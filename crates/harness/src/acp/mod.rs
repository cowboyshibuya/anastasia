...
fn claude_spec() -> AcpAgentSpec {
    AcpAgentSpec {
        ...
        install_hint: "claude-agent-acp (searched PATH, the login shell's PATH, npm \
             global bins, and fnm/nvm/volta/pnpm/bun install dirs; anastasia installs \
...
fn codex_spec() -> AcpAgentSpec {
    AcpAgentSpec {
        ...
        install_hint: "codex-acp (searched PATH, the login shell's PATH, npm global \
             bins, and fnm/nvm/volta/pnpm/bun install dirs; anastasia installs the \
...
fn gemini_spec() -> AcpAgentSpec {
    AcpAgentSpec {
        id: HarnessId::Gemini,
        display_name: "Gemini",
        executable: "gemini",
        env_override: "GEMINI_EXECUTABLE",
        args: &["--acp"],
        npm_package: None,
        extra_paths: || npm_global_bins("gemini"),
        cli_executable: "gemini",
        cli_extra_paths: || npm_global_bins("gemini"),
        install_hint: "gemini (searched PATH, the login shell's PATH, npm global bins, and fnm/nvm/volta/pnpm/bun install dirs; install Gemini CLI, authenticate normally, or set GEMINI_EXECUTABLE to override)",
        // Gemini advertises the account's current model/config over ACP.
        models: Vec::new,
...
fn pi_spec() -> AcpAgentSpec {
    AcpAgentSpec {
        ...
        install_hint: "pi-acp (searched PATH, the login shell's PATH, npm global bins, \
             and fnm/nvm/volta/pnpm/bun install dirs; anastasia installs the pinned \
        ...
        steering_mode: SteeringMode::TurnBoundary,
        // pi's thinking ladder (minimal→max; its extra "off" tier has no anastasia
        ...
        handle.spawn(async move {
            match crate::adapter_install::ensure_installed(pin, bin_name, display_name).await {
                Ok(entry) => tracing::info!(
                    target: "anastasia_harness::adapter_install",
...
impl AcpHarness {
    ...
    /// Gemini CLI's native ACP server (`gemini --acp`).
    pub fn gemini() -> Self {
        Self::with_spec(gemini_spec())
    }
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::debug!(target: "anastasia_harness::acp", "stderr: {line}");
                    tail.push(&line);
                }
            });
        }
...
/// Map an advertised `thought_level` value id onto anastasia's ladder.
...
/// anastasia's own (forced to the no-prompts choice), model rides the model rows,
...
/// catalogs (anastasia never declares the boolean config capability, so adapters
...
fn initialize_params(harness: HarnessId) -> Value {
    ...
    json!({
        ...
        // is what anastasia wants — the working tree is the source of truth for
...
/// bypassPermissions and the codex harness's approvalPolicy "never" (anastasia
...
fn handle_server_request(
    ...
    match method {
        ...
        // finished. Image generation has nowhere to land in a anastasia session.
        CURSOR_TASK => {
        ...
        CURSOR_GENERATE_IMAGE => {
            client.respond(
                &id,
                json!({ "outcome": { "outcome": "rejected", "reason": "anastasia cannot render generated images" } }),
            );
            Vec::new()
        }
        _ => {
            tracing::debug!(target: "anastasia_harness::acp", "unhandled server request: {method}");
...
/// one of these gets a response even when anastasia has nothing to do with it.
...
/// One `cursor/ask_question` entry: the anastasia-side question plus what is
/// needed to answer it — the wire id, and the label→optionId table (anastasia's
    ...
    params
        ...
        .filter_map(|q| {
            ...
            let choices: Vec<(String, String)> = q
                ...
                .collect();
            // An option-less question has no answer anastasia could send back.
            if choices.is_empty() {
                return None;
            }
            Some(CursorQuestion {
                wire_id,
                question: UserInputQuestion {
                    // Anastasia-minted: cursor's ids ("q1") repeat across turns.
                    id: new_message_id(),
...
/// unblocks without anastasia inventing a pick.
...
async fn run_session(session: Session) {
        ...
        res = tokio::time::timeout(handshake_timeout, setup) => {
            ...
            match res {
                ...
                Err(e) => {
                    ...
                    tracing::warn!(target: "anastasia_harness::acp", %error, "agent setup failed");
                    let _ = event_tx
    ...
    // `ANASTASIA_ACP_QUIET_SETTLE_MS` overrides; 0 disables.
    let quiet_settle: Option<Duration> = if cost_hint_enabled {
        None
    } else {
        match std::env::var("ANASTASIA_ACP_QUIET_SETTLE_MS")
            .ok()
...
mod tests {
    use super::*;
    use anastasia_proto::{TodoItem, ToolCall};
    ...
    /// keyed by cursor's wire ids, not the labels anastasia showed the user.
    #[test]
    fn cursor_questions_round_trip_labels_back_to_option_ids() {
        ...
        // Cursor's repeatable ids never leak into anastasia's question ids.
    ...
    fn cursor_todo_notifications_map_to_stable_chips() {
        ...
        // Everything else is noise anastasia has nothing to render for.
    ...
    #[test]
    fn gemini_spec_targets_the_native_acp_server() {
        let spec = gemini_spec();
        assert_eq!(spec.id, HarnessId::Gemini);
        assert_eq!(spec.executable, "gemini");
        assert_eq!(spec.args, &["--acp"]);
        assert!(spec.npm_package.is_none());
