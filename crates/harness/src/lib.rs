//! anastasia-harness — one interface over coding agents, all driven through the
...
pub struct RunControls {
    /// The run sends questions and awaits answers (blocks the agent, mirrors anastasia).
...
/// the proper background-crash message old anastasia showed (user requirement).
