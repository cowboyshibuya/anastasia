...
//!
//!   ANASTASIA_AB=1 cargo test -p anastasia-harness --test real_quiet_ab -- --ignored --nocapture
//!
...
use anastasia_harness::{AcpHarness, CancellationToken, Harness, RunControls, SteerMessage};
use anastasia_proto::{AgentEvent, RunRequest, SandboxLevel, UserInputAnswer, UserInputQuestion};
...
fn init_env() {
    ...
    ONCE.call_once(|| {
        ...
        unsafe { std::env::set_var("ANASTASIA_ACP_QUIET_SETTLE_MS", QUIET_MS.to_string()) };
    });
}
