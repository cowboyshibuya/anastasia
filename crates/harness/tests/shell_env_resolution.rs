...
use anastasia_harness::AcpHarness;
...
async fn cli_on_login_shell_path_only_is_resolved() {
    ...
    unsafe {
        ...
        std::env::remove_var("ANASTASIA_NO_LOGIN_SHELL");
    }

    let snapshot = anastasia_harness::shell_env::login_shell_path().expect("snapshot captured");
