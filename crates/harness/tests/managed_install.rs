...
//! `$ANASTASIA_ADAPTERS_DIR`, spawn it via node, and reach SessionStarted (the
...
//! `cargo test -p anastasia-harness --test managed_install -- --ignored`.
//!
//! Single-test binary: it mutates ANASTASIA_ADAPTERS_DIR process-wide.

use anastasia_harness::{AcpHarness, Harness, RunControls};
use anastasia_proto::{AgentEvent, RunRequest};
...
async fn managed_install_reaches_session_started() {
    ...
    unsafe {
        std::env::set_var("ANASTASIA_ADAPTERS_DIR", adapters.path());
    ...
    let version_dir = std::fs::read_dir(&root)
        ...
        .path();
    assert!(version_dir.join(".anastasia-install-ok").exists());
}
