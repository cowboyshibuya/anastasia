...
///
/// The `contribution` carries only what is Ponytail-specific: native plugin and
/// extension arguments, its environment, and its ruleset *text*. The text is not
/// turned into a command-line flag here — [`crate::harness::compose`] owns each
/// provider's single instruction channel, because Alabasta context wants the
/// same channel and two writers would silently overwrite one another.
...
impl PonytailLaunch {
    /// Ponytail is off. It contributes nothing, so the process launches exactly
...
fn resolve_claude(mode: PonytailMode) -> PonytailLaunch {
    ...
    PonytailLaunch::of(
        vec!["--plugin-dir".to_owned(), root.display().to_string()],
        vec![
        ...
        // The hooks carry the ruleset themselves; this line only makes the active
        // level legible to `/ponytail` when it reports back.
        Some(format!("Ponytail is active at level {}.", mode.id())),
...
fn resolve_pi(mode: PonytailMode) -> PonytailLaunch {
    ...
    PonytailLaunch::of(
        ...
        vec![("PONYTAIL_DEFAULT_MODE".to_owned(), mode.id().to_owned())],
        None,
        PonytailStatus::active(
            ProviderKind::Pi,
            mode,
...
";
    ...
    fn disabled_ponytail_contributes_nothing_to_the_launch() {
        ...
        assert!(PonytailLaunch::disabled().contribution.args.is_empty());
        assert!(PonytailLaunch::disabled().contribution.env.is_empty());
    ...
    fn providers_without_a_channel_are_unsupported_but_healthy() {
        for provider in [
            ...
            assert!(
                launch.contribution.is_empty(),
                "{provider:?} launch was modified"
            );
        }
    }
