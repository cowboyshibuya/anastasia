...
fn noop_nudge() -> anastasia_rpc::NudgeHandler {
    Arc::new(|_| {})
}
...
async fn nudges_reach_the_host_callback() {
    ...
    let on_nudge: anastasia_rpc::NudgeHandler = Arc::new(move |chat_id| {
...
/// `ANASTASIA_EDGE_WS=http://127.0.0.1:26640 cargo test -p anastasia-rpc -- --ignored live_edge`
/// (dev-mode edge; ANASTASIA_EDGE_TOKEN defaults to a fixed dev user id).
#[tokio::test]
#[ignore = "needs a running edge (set ANASTASIA_EDGE_WS)"]
async fn live_edge_relay_round_trip() {
    let Ok(edge_url) = std::env::var("ANASTASIA_EDGE_WS") else {
        panic!("set ANASTASIA_EDGE_WS to the edge base URL (e.g. http://127.0.0.1:26640)");
    };
    let token = std::env::var("ANASTASIA_EDGE_TOKEN").unwrap_or_else(|_| "relay-live-test".into());
