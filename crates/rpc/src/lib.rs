//! anastasia-rpc — the typed control plane (UiRpc / ControlRpc) over WebSocket + in-memory
...
//! byte transports), matching the shape of anastasia's Effect RPC without the Effect runtime:
//!
...
pub mod methods {
    ...
    /// Live sync introspection (`anastasia sync` / debug surfaces): per-room
