...
//! handler works through the relay untouched (the port of anastasia's `device-room-host.ts`).
...
/// so the superseded process backs off and retries, mirroring anastasia's DeviceRoomHost).
...
impl HostRelay {
    pub fn spawn(
        ...
        let task = tokio::spawn(async move {
            ...
            let mut online = anastasia_sync::wake::subscribe_online();
...
async fn host_session(
    ...
    let ws = anastasia_sync::dial::connect_ws(url)
        .await
...
pub struct LinkCacheConfig {
    ...
    /// redialed at full cadence; callers fail fast in between (anastasia peers.ts behavior).
...
impl LinkCacheConfig {
    pub fn new(edge_url: impl Into<String>, token: Arc<dyn TokenSource>) -> Self {
        ...
        // this cache: one blip must cost seconds, not minutes. The old anastasia
...
/// Lazily-dialed, cached peer links keyed by device id — the Rust twin of anastasia's
