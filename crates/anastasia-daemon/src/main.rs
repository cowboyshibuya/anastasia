...
use anastasia_protocol::{DAEMON_TOKEN_ENV, DaemonReady, PROTOCOL_VERSION};
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(parent_pid) = arguments.parent_pid {
        ...
        std::thread::Builder::new()
            .name("anastasia-daemon-parent".into())
            .spawn(move || {
    ...
    let task_path = anastasia_core::persistence::StateStore::default_path();
    let settings = anastasia_core::DaemonSettingsStore::open_with_legacy(
        anastasia_core::DaemonSettings::default_path(),
        [task_path.with_file_name("settings.json")],
    )
    .context("could not load daemon settings")?;
    let task_store = anastasia_core::persistence::StateStore::daemon(task_path);
    anastasia_core::serve(
        listener,
        token,
        Arc::new(anastasia_core::daemon::AnastasiaBackend::new(settings, task_store)?),
        shutdown,
        anastasia_core::ServerOptions {
