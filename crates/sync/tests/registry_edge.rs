...
//!
//! ```sh
//! ANASTASIA_EDGE_WS=ws://127.0.0.1:27640 cargo test -p anastasia-sync --test registry_edge -- --ignored
//! ```

use std::sync::{Arc, Mutex};
use std::time::Duration;

use anastasia_doc::RegistryDoc;
use anastasia_proto::{Chat, Session, SessionStatus};
use anastasia_sync::RegistryClient;
...
fn edge_url(org: &str, user: &str) -> String {
    let base = std::env::var("ANASTASIA_EDGE_WS")
        .expect("set ANASTASIA_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:27640");
...
#[tokio::test]
#[ignore = "requires a live edge: set ANASTASIA_EDGE_WS (e.g. ws://127.0.0.1:27640)"]
