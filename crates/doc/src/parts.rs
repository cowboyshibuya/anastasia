...
use anastasia_proto::{AgentEvent, ToolCall, ToolDiff, UserInputQuestion};
...
///
/// Semantics from anastasia `foldEventIntoParts`:
...
pub fn fold_event_into_parts(out: &mut Vec<MessagePart>, event: &AgentEvent) {
    match event {
        ...
        AgentEvent::ReasoningDelta { .. } => {
            // Reasoning is not rendered as a transcript part (matches anastasia).
        }
...
/// continuations (`continuation_id(root, i)`), matching `splitMessageEntry` in anastasia.
    ...
    fn session_started_resets_accumulator() {
        ...
        fold_event_into_parts(
            ...
            &AgentEvent::SessionStarted {
                harness: anastasia_proto::HarnessId::Mock,
                model: "m".into(),
                tools: vec![],
                cwd: "/".into(),
                session_id: "s".into(),
