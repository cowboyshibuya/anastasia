...
use anastasia_proto::{GitHistoryRefKind, TerminalEvent};
...
fn assemble(dir: &Path) -> EngineCore {
    ...
    EngineCore::assemble(
        ...
        anastasia_proto::HarnessId::Mock,
        None,
    )
...
async fn repos_round_trip_add_branches_worktrees() {
    ...
    // Worktree add: anastasia/<name> branch, isolated dir under the test root.
    let worktree = repos
        .create_worktree(&repo_dir, "main")
        .await
        .expect("worktree");
    assert!(
        worktree.branch.starts_with("anastasia/"),
        "branch: {}",
        worktree.branch
    );
    ...
    // worktree's anastasia/<name> branch maps to its linked-checkout path, and
    ...
    // Delete: dir removed, anastasia branch removed, refs pruned.
    repos
    ...
    assert!(
        ...
        "anastasia branch deleted: {branches:?}"
    );
...
async fn diff_file_text_returns_both_checked_sources() {
    ...
    let escape = anastasia_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
    ...
    assert!(snapshot.patch.contains("# Anastasia diff truncated"));
}
...
async fn rpc_dispatch_for_m5_methods() {
    ...
    unsafe { std::env::set_var("ANASTASIA_WORKTREES_DIR", tmp.path().join("worktrees")) };
    let core = assemble(&tmp.path().join("data"));
    let client = anastasia_rpc::memory_client(core.rpc_service());
