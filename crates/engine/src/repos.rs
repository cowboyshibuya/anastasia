...
//! browser (feature-inventory §3.5; port of anastasia's `repos.ts` + `folder-lister.ts`).
//!
...
//! created under `~/.anastasia/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `anastasia/<name>` branch. `ANASTASIA_WORKTREES_DIR` overrides the root.
//!
...
/// worktrees are user-facing working checkouts. `ANASTASIA_WORKTREES_DIR` overrides
/// (test isolation); empty reads as unset.
fn default_worktrees_root() -> PathBuf {
    std::env::var_os("ANASTASIA_WORKTREES_DIR")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".anastasia").join("worktrees"))
}
...
impl Repos {
    ...
    /// comes from `$ANASTASIA_WORKTREES_DIR` or `~/.anastasia/worktrees`.
    ...
    /// remotes, and tags seed the walk, so Anastasia's internal refs never leak
    ...
    /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `anastasia/<name>`
    /// branch off `branch`.
    pub async fn create_worktree(
        ...
        for attempt in 0..50u64 {
            ...
            if !base.join(&candidate).exists()
                && !existing.contains(&format!("anastasia/{candidate}"))
            {
                name = Some(candidate);
                break;
            }
        ...
        let branch_name = format!("anastasia/{name}");
        self.git(
            &[
                "worktree",
                "add",
    ...
    /// Rename a anastasia-created worktree branch after its chat's generated title
    /// (port of anastasia's `renameWorktreeBranch`). Guards:
    /// - respect an external checkout/rename: only act while the worktree is still
    ///   on `expected_branch` AND that branch is the original `anastasia/<folderName>`;
    ...
    pub async fn rename_worktree_branch(
        ...
        let folder = worktree_path
            ...
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        if current != expected_branch || expected_branch != format!("anastasia/{folder}") {
            return Ok(current);
        }
    ...
    /// Deletes the worktree's branch ONLY when anastasia created it (`anastasia/…`) — the
    ...
    pub async fn delete_worktree(
        ...
        if branch.starts_with("anastasia/") {
    ...
    /// timeout the thread is simply abandoned (the anastasia backend's disposable
...
fn list_folders_blocking(target: &Path) -> Result<FolderListing, EngineError> {
    let read = std::fs::read_dir(target).map_err(|e| match e.kind() {
        std::io::ErrorKind::PermissionDenied => {
            EngineError::Other("Anastasia doesn't have access to this folder on the device.".into())
        }
...
/// Turn a generated chat title into the semantic portion of a Anastasia branch
/// (port of anastasia's `worktreeBranchFromTitle`). Anastasia NFKD-normalizes accented
