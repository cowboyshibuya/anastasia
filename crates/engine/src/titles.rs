...
//! untitled chat, name it with the harness's cheapest model (port of anastasia's
...
//!    last listed model — anastasia's `cheapestModel`);
...
//!    retrying on anastasia's short backoff ladder; fall back to the prompt's first
//!    words when every attempt produces nothing;
//! 4. re-check the title (a user rename during generation wins);
//! 5. when the chat sits in a anastasia worktree (`anastasia/<name>` branch), rename the
...
use anastasia_harness::{CancellationToken, RunControls, SteerMessage};
use anastasia_proto::{
...
/// couple of times with a short backoff before falling back (anastasia's ladder).
...
impl TitleGenerator {
    ...
    async fn generate(
        ...
        // anastasia/<name> branch (guards live inside rename_worktree_branch).
        if let (Some(chat_cwd), Some(branch)) = (&latest.cwd, &latest.branch)
            && branch.starts_with("anastasia/")
        {
            match self
                .inner
                .repos
...
/// The cheapest model a harness offers (anastasia's `cheapestModel` heuristic):
...
async fn collect_text(
    harness: &dyn anastasia_harness::Harness,
    request: RunRequest,
