...
use anastasia_doc::{DeletedSpace, REGISTRY_DOC_ID, RegistryDoc, WorkspaceDoc};
use anastasia_proto::{Chat, ChatConfig, Device, Session, Space};
use anastasia_sync::{DocsStore, RegistryClient, RegistryTuning};
...
async fn token_revoked(token: &Option<Arc<dyn anastasia_rpc::TokenSource>>) -> bool {
    match token {
...
impl WorkspaceHost {
    ...
    pub fn connect_registry_url(&self, url: &str) {
        self.spawn_join(
            Arc::new(anastasia_sync::StaticUrl(url.to_string())),
            None,
            None,
        );
    }
    ...
    fn spawn_join(
        &self,
        url: Arc<dyn anastasia_sync::UrlProvider>,
        mut token_changes: Option<tokio::sync::watch::Receiver<u64>>,
        token: Option<Arc<dyn anastasia_rpc::TokenSource>>,
    ) {
        let org_id = self.inner.config.org_id.clone();
                        ...
                        loop {
                            tokio::select! {
                                event = events.recv() => match event {
                                    Ok(anastasia_sync::RegistryEvent::Applied)
                                    | Ok(anastasia_sync::RegistryEvent::Connected) => {
                                        let Some(inner) = weak.upgrade() else { return };
                                        inner.bump_changed();
                                    }
                                    Ok(anastasia_sync::RegistryEvent::Presence) => {
                                        let Some(inner) = weak.upgrade() else { return };
                                        inner.publish();
                                    }
                                    Ok(anastasia_sync::RegistryEvent::Disconnected) => {}
    ...
    /// Registry room introspection for SyncStatus / `anastasia sync`.
    /// `None` = no room yet (edge-less, or the initial join is still retrying).
    pub fn sync_status(&self) -> Option<anastasia_sync::RoomStatsSnapshot> {
    ...
    /// LWW full-config replace on the chat row (anastasia `SetChatConfig` — the
