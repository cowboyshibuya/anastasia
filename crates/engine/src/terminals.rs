...
//! anastasia's `terminals.ts` over `portable-pty`).
//!
...
//!   owner, so the per-user owner re-checks from anastasia's Router land with real
...
use anastasia_doc::TERMINAL_OUTPUT_BATCH_MS;
...
impl Terminals {
    ...
    pub fn open_with_shell(
        ...
        cmd.env("TERM_PROGRAM", "Anastasia");
        let mut child = pair
            .slave
            .spawn_command(cmd)
