//! Per-session on-disk event journal (port of anastasia's `run-journal.ts`, JSONL-shaped).
//!
...
use anastasia_proto::AgentEvent;
...
impl RunJournal {
    ...
    /// Auto-resume revival budget (anastasia `resumeAttempt`/`MAX_AUTO_RESUME`):
    ...
    /// from a previous era (file replaced) — falls back to a full replay, mirroring anastasia.
    pub fn replay(
        &self,
        chat_id: &str,
        after_seq: u64,
...
mod tests {
    use super::*;
    use anastasia_proto::DoneStatus;
