...
use crate::motion::{self, ANASTASIA_PULSE};
...
/// The floating-menu surface (anastasia `.glass-surface` + `menuSurface`):
...
/// One menu row (anastasia `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (anastasia's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (anastasia
...
/// Hairline divider between menu sections (anastasia `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (anastasia
...
/// A bordered trailing menu section (anastasia picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (anastasia dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (anastasia dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (anastasia:
