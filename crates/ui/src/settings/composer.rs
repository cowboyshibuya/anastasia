...
//! (anastasia parity: localStorage `anastasia.composer.defaults:v1`, defaults.ts).
//!
...
use anastasia_proto::{HarnessId, ReasoningLevel};

const FILE_NAME: &str = "composer-defaults.json";

/// Remembered model per harness — id plus display label, mirroring anastasia's
...
pub struct ComposerDefaults {
    ...
    /// Last reasoning level picked (global, like anastasia's `reasoning` key).
...
impl ComposerDefaults {
    ...
    /// Remember a pick (anastasia `saveDefaults({ harness, modelByHarness })`).
