...
use anastasia_proto::{Chat, CheckoutDiff, GitHistoryCommit};
...
pub struct DiffHighlights {
    pub old: Option<Arc<anastasia_syntax::HighlightedDocument>>,
    pub new: Option<Arc<anastasia_syntax::HighlightedDocument>>,
}
...
impl DiffHighlights {
    ...
    pub fn spans(&self, line: &DiffLine) -> &[anastasia_syntax::HighlightSpan] {
...
fn excerpt_side(
    ...
    for hunk in &file.hunks {
        ...
        let source = visible
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let document = anastasia_syntax::highlight(anastasia_syntax::HighlightRequest {
            source: &source,
            path: Some(path),
            fence_tag: None,
        })
    ...
    Some(Arc::new(anastasia_syntax::HighlightedDocument {
        language,
        lines,
    }))
}
...
fn excerpt_highlights(file: &FileDiff, language: Lang) -> Option<DiffHighlights> {
    if !anastasia_syntax::supports_language(language) {
        return None;
    }
...
fn sources_match_patch(file: &FileDiff, response: &anastasia_proto::CheckoutFileDiffText) -> bool {
    let old = response
        .old_text
        .as_deref()
...
fn full_highlights(
    file: &FileDiff,
    language: Lang,
    response: &anastasia_proto::CheckoutFileDiffText,
    ...
    if old.is_none() && new.is_none() && anastasia_syntax::supports_language(language) {
        return None;
    }
    Some(DiffHighlights { old, new })
}
...
impl Changes {
            ...
            this.update(cx, |changes, cx| {
                ...
                match result.and_then(|value| {
                    serde_json::from_value::<CheckoutDiff>(value)
                        .map_err(|e| anastasia_rpc::RpcError::Failed(e.to_string()))
                }) {
                    Ok(diff) => {
    ...
    fn request_highlight(
        ...
        if !anastasia_syntax::supports_language(lang) {
        ...
        let fetch_task = match (active, engine) {
            (Some(diff), Some(engine)) => Some(cx.spawn(async move |this, cx| {
                let request = anastasia_proto::GetCheckoutFileDiffTextRequest {
                ...
                let response = engine
                    ...
                    .ok()
                    .and_then(|value| {
                        serde_json::from_value::<anastasia_proto::CheckoutFileDiffText>(value).ok()
                    });
    ...
    fn render_file_header(
        ...
        // Chevron (anastasia checkout-diff-sidebar): chevron-right closed,
...
fn diff_line_row(
    line: &DiffLine,
    spans: &[anastasia_syntax::HighlightSpan],
    theme: &Theme,
    gutter_px: f32,
) -> AnyElement {
...
/// column, and paint-only syntax runs (anastasia checkout-diff-sidebar).
        ...
        let scoped_notice: Option<(SharedString, bool)> = (!no_chat
            ...
            .map(|message| {
                if message.contains("no turn recorded") {
                    ...
                    (
                        SharedString::from(
                            "This chat's device is running an older Anastasia — update it to view branch and turn diffs",
                        ),
                        false,
                    )
                } else {
...
";
    ...
    fn phases() {
        ...
        summarized.files.push(anastasia_proto::DiffFileSummary {
            path: "x".into(),
            old_path: None,
    ...
    fn full_diff_highlights_map_old_new_and_context_by_source_line() {
        ...
        assert!(
            highlights
                .spans(&deleted)
                .iter()
                .any(|span| span.kind == anastasia_syntax::HighlightKind::Function)
        );
        assert!(
            highlights
                .spans(&added)
    ...
    fn excerpt_parses_old_and_new_hunks_as_separate_documents() {
        ...
        assert!(
            highlights
                .spans(deleted)
                .iter()
                .any(|span| span.kind == anastasia_syntax::HighlightKind::Comment)
        );
        assert!(
            highlights
                .spans(added)
    ...
    fn mismatched_full_sources_are_rejected_atomically() {
        ...
        let response = anastasia_proto::CheckoutFileDiffText {
