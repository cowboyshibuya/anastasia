...
//!
//! The set mirrors the original anastasia's icon usage exactly:
...
//!   are ports of the hand-drawn inline SVGs in anastasia's `terminal-panel.tsx` /
//!   `composer-actions.tsx`.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   ports of anastasia's `icons.tsx`. gpui tints SVGs with the text color, so the
...
icon_assets![
    ...
    // Mirrored variant (anastasia window-controls.tsx `-scale-x-100`): the LEFT
    ...
    // Hand-drawn anastasia glyphs (terminal-panel.tsx / composer-actions.tsx /
...
/// The Claude mark's brand orange (`#D97757`) — anastasia keeps it even on the
/// monochrome surface.
