//! Animation kit — the anastasia motion catalog as reusable helpers over gpui
...
//! - `anastasia-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// anastasia's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
/// (`transition-colors` et al. carry it unless overridden; anastasia never does).
...
/// Anastasia loader pulse period: 2.4s.
pub const ANASTASIA_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
...
/// (anastasia also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Anastasia-pulse floor opacity.
// The loader constants and math live in `anastasia_proto::motion` (pure phase
...
// the pointer enters. The original anastasia puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`ANASTASIA_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `ANASTASIA_MOTION_SCALE=10` slows the
...
mod tests {
    ...
    #[test]
    fn catalog_timings_match_anastasia() {
        ...
        assert_eq!(ANASTASIA_PULSE.duration_ms, 2400);
