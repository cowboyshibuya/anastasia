...
//!
//! Ports of anastasia's `composer/use-attachments.ts` (staging/upload),
...
/// Base64 chars per `UploadChunk` (anastasia state.ts `UPLOAD_CHUNK` — sized for
...
/// `(body_end, refs_start)` byte offsets — the tolerant equivalent of anastasia's
/// `ATTACHED_IMAGES_RE`.
...
/// the same stuck-offset guard as anastasia's `readAttachmentImage`).
