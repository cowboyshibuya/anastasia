...
use anastasia_proto::{ChatIndicator, Device, FolderListing, Space};
...
impl Shell {
    ...
    pub(super) fn render_active_rows(
        ...
        let now = Utc::now();
        let filter = self.settings.space_filter.clone();
        let rows: Vec<(ChatIndicator, anastasia_proto::Chat, String, Option<String>)> = {
            let state = self.state.read(cx);
            state
                .overview_chats(now)
                .into_iter()
    ...
    pub(super) fn render_archived_section(
        ...
        let now = Utc::now();
        let filter = self.settings.space_filter.clone();
        let rows: Vec<anastasia_proto::Chat> = {
            let state = self.state.read(cx);
            state
                .chats
                .iter()
    ...
    fn add_space_filtered(&self, cx: &App) -> Vec<anastasia_proto::FolderEntry> {
    ...
    pub(super) fn render_add_space_overlay(
        ...
        // ── breadcrumbs ("MacBook Pro / Projects / anastasia"): the quiet mono
