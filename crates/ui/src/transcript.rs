...
use anastasia_syntax::LanguageId as Lang;
...
/// Transcript column max width (anastasia 46rem).
pub const MAX_CONTENT_WIDTH: f32 = 736.0;
/// Tool chip row height / gap — analytic, so fold heights need no measurement.
/// A row is the guide rail + a 30px chip card centered in it (anastasia
...
pub enum ToolDetail {
    ...
    Stats {
        stats: Arc<Vec<anastasia_doc::ToolDiffStat>>,
    },
}
...
pub fn tool_detail(
    output: Option<&str>,
    diff: Option<&anastasia_proto::ToolDiff>,
    diff_stats: Option<&[anastasia_doc::ToolDiffStat]>,
) -> Option<ToolDetail> {
...
/// Reduce an inline [`anastasia_proto::ToolDiff`] to the changes pane's
...
pub fn diff_to_file(diff: &anastasia_proto::ToolDiff) -> crate::changes::FileDiff {
...
pub struct Row {
    ...
    /// its timestamp strip (anastasia chat-view.tsx `group`/`group-hover`).
...
/// `ANASTASIA_FRAME_STATS=1` logs live-row render-cost percentiles (p50/p95 µs
...
fn frame_stats_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        std::env::var("ANASTASIA_FRAME_STATS").is_ok_and(|v| !v.is_empty() && v != "0")
    })
}

const FRAME_STATS_WINDOW: usize = 240;

/// `ANASTASIA_NO_RENDER_CACHE=1` bypasses the cross-frame flatten cache — the
/// A/B knob for the frame-cost measurement above.
fn render_cache_disabled() -> bool {
    static DISABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *DISABLED.get_or_init(|| {
        std::env::var("ANASTASIA_NO_RENDER_CACHE").is_ok_and(|v| !v.is_empty() && v != "0")
    })
}
...
///
/// The rule lives in `anastasia_proto::view` so the terminal viewport reports the
...
pub fn tool_group_summary(tools: &[ToolItem]) -> String {
    ...
    anastasia_proto::view::tool_group_summary(&pairs)
}

// `single_line` and the per-kind chip label/detail are shared with the terminal
// viewport (`anastasia_proto::view`): a tool must be named identically on every
...
// cell grid).
pub use anastasia_proto::view::{single_line, tool_chip_content};
...
fn blob_detail(text: &str, is_diff: bool) -> Option<ToolDetail> {
    if is_diff {
        let diff: anastasia_proto::ToolDiff = serde_json::from_str(text).ok()?;
...
struct HighlightEntry {
    ...
    document: Option<Weak<anastasia_syntax::HighlightedDocument>>,
    _task: Option<Task<()>>,
}
...
impl HighlightStore {
    ...
    fn request(
    ...
    ) -> Option<Arc<anastasia_syntax::HighlightedDocument>> {
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let document = cx
                .background_executor()
                .spawn(async move {
                    anastasia_syntax::highlight(anastasia_syntax::HighlightRequest {
                        source: &code,
                        path: None,
...
pub struct Transcript {
    ...
    /// strip (anastasia chat-view.tsx `group-hover`; the rows report hover
...
impl Transcript {
    ...
    fn spawn_blob_fetch(&mut self, blob_ref: SharedString, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let reply = crate::attachments::call_with_timeout(
                ...
                anastasia_rpc::methods::FETCH_TOOL_BLOB,
    ...
    /// device (uploads targeted it) plus this device (anastasia's
    ...
    fn render_row(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        ...
        // Hover-revealed timestamp strip (anastasia chat-view.tsx `Timestamp`):
        ...
        div()
            ...
            .pt(px(top_gap))
            .pb(px(bottom_pad))
            // Wide gutters (anastasia `px-4 @3xl:px-12`) around the 46rem column.
            .px(px(48.0))
            .child(
                div()
                    .w_full()
    ...
    fn code_highlight_for(
        ...
        only: Option<usize>,
        cx: &mut Context<Self>,
    ) -> HashMap<usize, Option<Arc<anastasia_syntax::HighlightedDocument>>> {
        ...
        for (ix, top) in tree.blocks.iter().enumerate() {
            ...
            if let Block::CodeBlock { language, code } = &top.block
                && let Some(lang) = language
                    .as_deref()
                    .and_then(anastasia_syntax::language_for_alias)
            {
                out.insert(
                    ix,
    ...
    fn tool_diff_highlight_for(
        ...
        let old = match old_text {
            Some(source) => {
                ...
                let lang = anastasia_syntax::language_for_path(path)?;
                Some(
                    self.highlights
        ...
        let new = match new_text {
            Some(source) => {
                let lang = anastasia_syntax::language_for_path(&file.path)?;
    ...
    fn render_tool_group(
        ...
        // Header (anastasia tool-group.tsx): a small chevron tile centered over the
        ...
        let header = div()
            ...
            // chips (destructive tint, anastasia tool-chip.tsx) and in the
...
/// The transcript ErrorChip — a port of anastasia chat-view.tsx `ErrorChip`
...
/// A small glyph standing in for the tool's icon (anastasia uses an icon set; a
/// quiet monochrome character keeps the tile without shipping SVGs).
/// The glyph for a tool call (anastasia tool-chip.tsx `toolIcon`, Solar set).
...
mod tests {
    ...
    fn user_rows_project_file_mentions_into_chips() {
        let raw = "look at [composer.rs](anastasia-file:crates/ui/src/composer.rs) please";
    ...
    fn tool_diff_builds_real_hunks_with_context_and_numbers() {
        ...
        new[9] = "LINE 10".into();
        let diff = anastasia_proto::ToolDiff {
            path: "/w/a.rs".into(),
        ...
        let created = anastasia_proto::ToolDiff {
            path: "/w/new.txt".into(),
            old_text: None,
            new_text: "only\n".into(),
        };
    ...
    fn call_block_carries_the_full_invocation() {
        ...
        let Some(ToolDetail::Output { lines, .. }) = call_block(&ToolCall::Mcp {
            server: "gh".into(),
            tool: "issues".into(),
            input: Some(serde_json::json!({"repo": "anastasia"})),
        }) else {
            panic!("expected an output block")
        };
        assert_eq!(lines[0].as_ref(), "gh · issues");
        assert!(lines.iter().any(|l| l.contains("\"repo\": \"anastasia\"")));
