...
//! lightness steps; the same scale anastasia's Tailwind theme used) into gpui [`Hsla`].
...
use anastasia_syntax::HighlightKind;
...
impl Theme {
    ...
    /// Main-panel header height (anastasia `h-11`) — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet (anastasia `h-6`) — the
    ...
    /// (see [`crate::frost::frosted`]). Dark: the reference anastasia
...
mod tests {
    ...
    fn neutral_950_is_0a0a0a() {
        // oklch(0.145 0 0) is Tailwind neutral-950, anastasia's app background.
    ...
    #[test]
    fn layout_numbers_match_anastasia() {
