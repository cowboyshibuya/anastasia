//! The app shell (anastasia `__root.tsx`): sidebar column + main panel + optional
//! right "Changes" pane, plus the boot splash and the connection gate.
//!
//! Layout is anastasia's: collapsible drag-resizable sidebar (208–400px, default
...
use anastasia_engine::InstanceLock;
use anastasia_proto::{AuthState, WorkspaceScope};
...
/// left edge (anastasia window-controls.tsx: `left: fullscreen ? 12 : 88`). The
...
pub fn apply_keymap(cx: &mut App, keymap: &KeymapConfig) {
    ...
    cx.bind_keys([
        KeyBinding::new(
            &valid_or_default(&keymap.open_settings, "mod-,"),
            OpenSettings,
            None,
        ),
        KeyBinding::new(
        ...
        KeyBinding::new(
            &valid_or_default(&keymap.toggle_changes, "mod-i"),
            ToggleChanges,
            None,
        ),
        KeyBinding::new(
...
impl SettingsSection {
    ...
    /// Sidebar + header label (anastasia settings-sidebar.tsx SECTIONS / __root.tsx
...
/// Per-chat panel open flags (anastasia parity: `sessionPanels` — the terminal and
...
/// One route-history entry (anastasia parity: the renderer's TanStack memory
...
/// (anastasia window-controls.tsx semantics): every route change pushes an entry;
...
impl NavHistory {
    ...
    /// equivalent of a `replace: true` navigation (anastasia's boot redirect from
    ...
    /// "can go forward" (anastasia window-controls.tsx).
...
pub struct Shell {
    ...
    sound_prev: std::collections::HashMap<String, anastasia_proto::SessionStatus>,
    ...
    install: anastasia_update::InstallKind,
    ...
    /// Dev/testing knobs (`ANASTASIA_OPEN_DIALOG`, `ANASTASIA_FORCE_GATE`) — see
    /// [`Shell::new`].
...
impl Shell {
    pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
        ...
        // Dev/testing knob: `ANASTASIA_OPEN_ROUTE=settings[/<section>]` boots
        // straight into a settings section — these pages have no deep link and
        // synthetic input can't reach them on headless compositors.
        let route = match std::env::var("ANASTASIA_OPEN_ROUTE").ok().as_deref() {
        ...
        // More capture knobs of the same kind: `ANASTASIA_OPEN_DIALOG=rename|delete`
        // opens that dialog for the first chat once chats land; `=model` pops
        // the combined harness/model menu once the shell is Ready;
        // `ANASTASIA_FORCE_GATE=signin|org|failed` renders that gate regardless of
        // real auth state (display-only — for styling passes).
        let debug_dialog = std::env::var("ANASTASIA_OPEN_DIALOG").ok();
        let debug_gate = match std::env::var("ANASTASIA_FORCE_GATE").ok().as_deref() {
            Some("signin") => Some(GatePhase::SignIn),
            Some("org") => Some(GatePhase::OrgGate),
            Some("failed") => Some(GatePhase::Failed(
                "Could not reach the anastasia engine on port 27901".into(),
            )),
            _ => None,
        };
        ...
        Self {
            ...
            install: anastasia_update::detect_install(),
            org: None,
    ...
    fn on_state_changed(&mut self, state: &Entity<AppState>, cx: &mut Context<Self>) {
        ...
        {
            let now = Utc::now();
            type Ping = (String, anastasia_proto::SessionStatus, bool, Option<String>);
            let sessions: Vec<Ping> = {
                ...
                state
                    ...
                    .iter()
                    .map(|s| {
                        use anastasia_proto::view::Indicator;
                        let status = match anastasia_proto::view::effective_indicator(Some(s), now)
                        {
                            Indicator::Working => anastasia_proto::SessionStatus::Working,
                            Indicator::AwaitingInput => {
                                anastasia_proto::SessionStatus::AwaitingInput
                            }
                            Indicator::Errored => anastasia_proto::SessionStatus::Errored,
                            Indicator::None => anastasia_proto::SessionStatus::Idle,
                        };
            ...
            // Anastasia window being key), so a ping for a *background chat* in a
            // focused app still stays a chime — you're already looking at
            // Anastasia; the sidebar dot carries the rest.
        ...
        if selected != self.active_chat {
            ...
            // anastasia's `/` route redirected into the last-used chat, leaving no
    ...
    fn add_commit_diff_surface(
        &mut self,
        commit: anastasia_proto::GitHistoryCommit,
        cx: &mut Context<Self>,
    ) {
    ...
    /// The flag is per chat (anastasia `sessionPanels`).
    fn toggle_terminal(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if open {
            ...
            // to the prompt, no click needed (anastasia terminal-panel.tsx: the
            ...
            // press closes even while the terminal is focused, as in anastasia's
    ...
    fn on_right_pane_drag(
        ...
        // anastasia caps the pane at 52% of the window on top of the absolute range.
                ...
                match result {
                    ...
                    Err(err) => {
                        shell.runtime_change_error = Some(format!(
                            "Could not stop the remote engine: {err}. Run `anastasia daemon stop`, then quit and reopen Anastasia."
                        ).into());
                        cx.notify();
                    }
                }
    ...
    /// reduced motion: exactly `target`. Honors `ANASTASIA_MOTION_SCALE`.
    ...
    /// of anastasia __root.tsx `transition-[padding-left] duration-200 ease-out` +
    ...
    /// by every route transition (anastasia remounts the keyed header variants —
    ...
    /// pattern (anastasia's `.drag` region): mark it a [`WindowControlArea::Drag`]
    ...
    /// anastasia window-controls.tsx): rendered once, in a paint-only overlay layer
    ...
    /// Native Windows caption controls integrated into Anastasia's unified
            ...
            .child(
                div().px(px(Theme::SPACE_SM)).pb(px(12.0)).child(
                    div()
                        ...
                        .child(
                            // AltArrowLeft chevron (anastasia settings-sidebar.tsx),
    ...
    /// One session row (anastasia session-row.tsx): status rail on the left
    ...
    fn render_chat_row(
        ...
        harness: Option<anastasia_proto::HarnessId>,
        status: anastasia_proto::ChatIndicator,
        selected: bool,
        archived: bool,
        theme: &Theme,
        cx: &mut Context<Self>,
        ...
        let status_label: Option<&'static str> = match status {
            anastasia_proto::ChatIndicator::Working => Some("Working"),
            anastasia_proto::ChatIndicator::AwaitingInput => Some("Input"),
            anastasia_proto::ChatIndicator::Errored => Some("Failed"),
            anastasia_proto::ChatIndicator::Completed => Some("Done"),
            anastasia_proto::ChatIndicator::Idle => None,
        };
        let corner_body: AnyElement = if corner_hovered {
            ...
            match status_label {
                Some(label) => {
                    ...
                    let glyph: AnyElement = if status == anastasia_proto::ChatIndicator::Completed {
                        icon(icons::CHECK)
                            .size(px(11.0))
                            .flex_none()
        ...
        // Hover fades over transition-colors (anastasia session-row.tsx) — both
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .when(status == anastasia_proto::ChatIndicator::Working, |el| {
                        el.child(div().flex_1())
    ...
    fn render_chat_sidebar(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        div()
            ...
            // Update strip (above the footer; below the lists).
            ...
            .child(
                div().px(px(Theme::SPACE_SM)).pt(px(Theme::SPACE_SM)).child(
                    div()
                        .id("sidebar-settings")
                        .flex()
                        .flex_row()
    ...
    /// (`anastasia update`); click dismisses it for that version.
    fn render_update_strip(&mut self, theme: &Theme, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let mac_app = matches!(self.install, anastasia_update::InstallKind::MacApp { .. });
        ...
        let (label, clickable): (SharedString, bool) = if mac_app {
            ...
            (
                format!("Update available — v{latest} · run `anastasia update`").into(),
                true,
            )
        };
    ...
    fn on_update_strip_click(&mut self, cx: &mut Context<Self>) {
        if !matches!(self.install, anastasia_update::InstallKind::MacApp { .. }) {
    ...
    /// Fetch the manifest and stage the new Anastasia desktop bundle under the data dir
    /// (tokio — reqwest); the strip flips to "restart to apply" when done.
    fn begin_update_download(&mut self, cx: &mut Context<Self>) {
        let release_url = anastasia_update::release_api_url();
        let data_dir = self.data_dir.clone();
        self.update_flow = UpdateFlow::Downloading;
        let download = Tokio::spawn(cx, async move {
            let manifest = anastasia_update::fetch_latest(&release_url).await?;
            anastasia_update::stage_mac_app(&release_url, &manifest, &data_dir).await
        });
    ...
    fn apply_staged_update(&mut self, staged: PathBuf, cx: &mut Context<Self>) {
        let anastasia_update::InstallKind::MacApp { bundle } = self.install.clone() else {
            return;
        };
        match anastasia_update::apply_mac_app(&staged, &bundle) {
            Ok(()) => {
                anastasia_update::relaunch_app_after_exit(&bundle);
                cx.quit();
            }
            Err(err) => {
    ...
    fn render_user_menu(
        ...
        let mut trigger = div()
            ...
            .child(
                // Avatar: white circle, initial in near-black (anastasia user-menu.tsx).
                div()
                    .size(px(28.0))
                    .flex_none()
                    .rounded_full()
        ...
        if self.user_menu.get().is_some() {
            ...
            // then the action selected by the runtime scope.
        ...
        let card = match self.sync_flow {
            SyncFlow::Enabling => popover::dialog_card(&theme)
                ...
                .child(
                    div().mt(px(6.0)).child(popover::dialog_body(
                        &theme,
                        "Finish signing in in your browser. Anastasia will keep using this local workspace until you quit and reopen.",
                    )),
                )
                .child(
                    div()
            ...
            SyncFlow::SwitchOffer { notice_open: true } => {
                ...
                let body: SharedString = match (&signed_in_email, &work_phrase) {
                    ...
                    (Some(email), None) => format!(
                        "You're signed in as {email}. Anastasia can switch to your synced workspace now."
                    )
                    .into(),
                    ...
                    .into(),
                    (None, None) => "Anastasia can switch to your synced workspace now.".into(),
                };
                let mut actions = div()
                    .mt(px(16.0))
                    .flex()
            ...
            SyncFlow::RestartPending { notice_open: true } => popover::dialog_card(&theme)
                ...
                .child(
                    div().mt(px(6.0)).child(popover::dialog_body(
                        &theme,
                        if remote_engine {
                            "Anastasia is using a background daemon. Stop it and quit Anastasia, then reopen to start the synced workspace. Existing local sessions stay on this device and will not be uploaded."
                        } else {
                            "Quit and reopen Anastasia to start the synced workspace. Existing local sessions stay on this device and will not be uploaded."
                        },
                    )),
                )
                .when_some(self.runtime_change_error.clone(), |card, error| {
            ...
            SyncFlow::SignOutConfirm => popover::dialog_card(&theme)
                ...
                .child(
                    div().mt(px(6.0)).child(popover::dialog_body(
                        &theme,
                        "Anastasia will remove your credentials, close the synced workspace, and continue in local mode.",
                    )),
                )
                .child(
                    div()
    ...
    fn render_main(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let outlet: AnyElement = if has_selection {
            ...
            // New-chat canvas (anastasia index.tsx): the anastasia mark over the
    ...
    fn render_status_strip(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // inner gutter (anastasia's `mx-auto h-6 max-w-3xl px-2`).
        let strip = div()
        ...
        let card = div()
            ...
            .child(
                div()
                    ...
                    .text_color(theme.text_muted)
                    .child(SharedString::from(
                        "Anastasia removed your credentials but could not finish closing the previous synced workspace. Retry before continuing in local mode.",
                    )),
            )
            .when_some(self.runtime_change_error.clone(), |card, error| {
                card.child(
    ...
    fn render_gate_card(&mut self, phase: &GatePhase, cx: &mut Context<Self>) -> AnyElement {
        ...
        let content: AnyElement = match phase {
            // Backend unreachable: quiet centered copy (anastasia Gate `Failed`),
            ...
            GatePhase::Failed(error) => div()
                ...
                .into_any_element(),
            // Login card (anastasia App.tsx Gate): centered card on the grid —
            // logo, "Log in to Anastasia", copy, full-width white Log in button.
            _ => div()
                .w(px(360.0))
                .px(px(32.0))
                .py(px(40.0))
                ...
                .child(
                    div()
                        ...
                        .font_weight(gpui::FontWeight::SEMIBOLD)
                        .text_color(theme.text)
                        .child(SharedString::from("Log in to Anastasia")),
                )
                .child(
                    div()
                        .mt(px(6.0))
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .items_center()
                    .justify_center()
                    // Keyed per phase (anastasia App.tsx `<div key={phase}
    ...
    fn render_org_gate(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // anastasia App.tsx OrgGate: w-400 card on the grid — logo, headline,
        ...
        let blurb: SharedString = match email {
            Some(email) => format!(
                "Anastasia is organized around workspaces — create one for yourself or your team. Signed in as {email}."
            )
            .into(),
            None => {
                "Anastasia is organized around workspaces — create one for yourself or your team."
                    .into()
            }
        };
        let card = div()
...
/// The sign-in gate's faint grid backdrop (anastasia styles.css `.bg-grid`):
...
/// A size-6 icon button for the titlebar strip (anastasia window-controls.tsx:
...
fn window_control_button(
    ...
    div()
        ...
        .rounded(px(6.0))
        .cursor_pointer()
        // anastasia window-controls.tsx: `transition-colors` — the wash fades.
        .bg(motion::hover_blend(
            &fade_key,
            theme.glass_hover().opacity(0.0),
            theme.glass_hover(),
...
/// A titlebar history button (anastasia window-controls.tsx): enabled it is a
...
/// A size-7 icon button for the main-panel header (anastasia __root.tsx:
...
fn header_icon_button(
    ...
    div()
        ...
        .rounded(px(6.0))
        .cursor_pointer()
        // anastasia __root.tsx header buttons: `transition-colors`.
        .bg(motion::hover_blend(
            &fade_key,
            crate::theme::wash(0.0),
            crate::theme::wash(0.11),
...
impl Render for Shell {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = Theme::of(cx);
        // The shell tone (anastasia `.frost`): the surface the sidebar sits on and
        ...
        let root = div()
            ...
            // no-ops (anastasia __root.tsx gates the hotkey on `!isSettings`, and
        ...
        let root = match &render_gate {
            GatePhase::Ready => {
                ...
                // Capture knob: `ANASTASIA_OPEN_DIALOG=model` pops the combined
                ...
                // never renders it (anastasia __root.tsx `!isSettings && activeChat`
                ...
                let card: AnyElement = div()
                    ...
                    .child(main)
                    .into_any_element();
                // The whole app page is one keyed `animate-in` entrance (anastasia
...
mod tests {
    ...
    async fn signed_out_synced_runtime_stops_and_reboots_local() {
        ...
        let boot = EngineBootConfig {
            ...
            default_harness: anastasia_proto::HarnessId::Mock,
        };
    ...
    fn local_sign_in_offers_the_in_place_switch() {
        let signed_in = AuthState::SignedIn {
            user: anastasia_proto::UserProfile {
                id: "user-1".into(),
    ...
    #[test]
    fn titlebar_cluster_matches_anastasia_window_controls() {
        // anastasia window-controls.tsx: `left: fullscreen ? 12 : 88` — the
    ...
    // ---- per-session panel flags (§1.10/1.11 parity: anastasia sessionPanels) ----
    ...
    fn nav_push_truncates_the_forward_branch() {
        ...
        // semantics — anastasia's memory history PUSH truncates entries ahead).
        let mut nav = NavHistory::new(chat("a"));
        nav.push(chat("b"));
        nav.push(chat("c"));
        nav.back();
