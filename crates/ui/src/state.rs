...
//! port, mirroring anastasia: if an engine is listening it connects over WebSocket
...
use anastasia_doc::{SessionMessageEntry, TranscriptDesync, TranscriptFrame};
use anastasia_engine::{Engine, EngineConfig, EngineRuntime, InstanceLock, rpc::AuthRpc};
use anastasia_proto::{
    AuthState, Chat, ChatIndicator, Device, EngineInfo, HarnessId, Session, Space, WorkspaceScope,
};
use anastasia_rpc::{
    RpcClient, RpcError, RpcReply, RpcService, connect_ws, memory_client, methods,
};
...
pub struct EngineBootConfig {
    /// Data directory for the embedded engine (`~/.anastasia`).
...
impl EngineHandle {
    ...
    pub async fn bootstrap(config: EngineBootConfig) -> anyhow::Result<EngineHandle> {
        ...
        let ipc_task = match anastasia_engine::serve_ipc(engine_config.ipc_port, service).await {
            Ok(task) => Some(task),
            Err(err) => {
                tracing::warn!(
...
pub struct AppState {
    ...
    pub update: Option<anastasia_update::UpdateStatus>,
...
impl AppState {
    ...
    pub fn apply_chat_config(&mut self, chat_id: &str, config: anastasia_proto::ChatConfig) {
    ...
    pub fn apply_update(&mut self, status: anastasia_update::UpdateStatus) {
    ...
    pub fn auth_user(&self) -> Option<&anastasia_proto::UserProfile> {
    ...
    pub fn apply_transcript_frame(
        ...
        anastasia_doc::apply_transcript_frame(&mut self.transcript, frame)?;
        if let Some(chat_id) = self.selected_chat.as_deref()
            && let Some(echoes) = self.echoes.get_mut(chat_id)
        {
...
mod tests {
    use super::*;
    use anastasia_engine::{EngineCore, default_registry};
    use chrono::TimeDelta;
    // `SessionStatus` is only needed to build the fixtures below — the module
    // itself derives everything through `anastasia_proto::view`.
    use anastasia_proto::{SessionStatus, UserProfile};
    ...
    async fn remote_viewport_treats_legacy_daemon_as_ready() {
        ...
        let server = tokio::spawn(anastasia_rpc::serve_ws_listener(
            listener,
    ...
    async fn bootstrap_reports_local_assembly_failure_before_returning_a_handle() {
        let dir = tempfile::tempdir().unwrap();
        anastasia_engine::EngineProfile::local(dir.path()).unwrap();
    ...
    async fn bootstrap_connects_when_daemon_is_listening() {
        // Stand in for `anastasia headless`: an engine served over the WS IPC port.
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        tokio::spawn(anastasia_rpc::serve_ws_listener(
            listener,
            core.rpc_service(),
        ));
    ...
    fn apply_chat_config_stamps_the_row() {
        ...
        let config = anastasia_proto::ChatConfig {
            ...
            reasoning: Some(anastasia_proto::ReasoningLevel::XHigh),
    ...
    fn project_labels_from_cwd() {
        assert_eq!(project_label(Some("/home/w/dev/anastasia")), "anastasia");
        assert_eq!(project_label(Some("/home/w/dev/anastasia/")), "anastasia");
    ...
    fn grouped_sidebar_preserves_recency_order() {
        ...
        let chats = [
            chat_with_cwd("a", 9, Some("/dev/anastasia")),
            chat_with_cwd("b", 8, Some("/dev/zed")),
            chat_with_cwd("c", 7, Some("/dev/anastasia")),
        ...
        assert_eq!(labels, ["anastasia", "zed", "No project"]);
        let anastasia_ids: Vec<&str> = groups[0].chats.iter().map(|c| c.id.as_str()).collect();
        assert_eq!(anastasia_ids, ["a", "c"]);
        assert!(group_chats(std::iter::empty()).is_empty());
    }

    #[test]
    fn relative_times_match_anastasia_format() {
        let now = Utc::now();
    ...
    fn chat_location_joins_project_and_branch() {
        ...
        c.branch = Some("anastasia/rebalance".into());
        assert_eq!(
            chat_location(&c).as_deref(),
            Some("soccertcg · anastasia/rebalance")
        );
        c.branch = None;
