...
//! `remend`, ported to anastasia's incremental parser).
//!
...
//! handlers don't apply here — anastasia renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "anastasia:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](anastasia:pending-link)");
        mends("[docs](", "[docs](anastasia:pending-link)");
        mends("see [do", "see [do](anastasia:pending-link)");
        mends("![alt](https://x/i.p", "![alt](anastasia:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](anastasia:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](anastasia:pending-link)");
        mends("**a [b", "**a [b](anastasia:pending-link)**");
    }
