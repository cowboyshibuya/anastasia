...
- Add an **ACP harness** (`crates/harness/src/acp/`) speaking Agent Client Protocol
  ...
  NOT the official SDK crates: anastasia keeps its own child-lifecycle hardening
...
- First registered agent: **Grok Build** (`grok agent stdio`), xAI's native ACP
  ...
  OAuth or `XAI_API_KEY`; anastasia passes env through. `GROK_EXECUTABLE` overrides
...
- **claude/codex converted to ACP too** (2026-08-08, wing's call: "keep things
  ...
  adapter-owned; anastasia-specific settings ride config options where advertised
...
- **Hermes + Pi registered** (2026-08-08): `AcpHarness::hermes()` runs Nous
  ...
  entry); thinking ladder minimal→max maps onto anastasia's levels via the
  generic `thought_level` preference ladder ("off" has no anastasia tier).
...
- **Session config options**: ACP has no per-prompt model field; the run's
  ...
  `low`/`medium`/`high`; other anastasia levels degrade down a preference ladder
  (`config_option_sets`).
...
- Replaced by `adapter_install`: pinned packages install ONCE into
  `~/.anastasia/adapters/<pkg>/<version>` (`$ANASTASIA_ADAPTERS_DIR` overrides) with
  a anastasia-owned npm cache beside them, atomically (tmp dir + bin-entry
  verification + `.anastasia-install-ok` marker + rename), then every launch is
