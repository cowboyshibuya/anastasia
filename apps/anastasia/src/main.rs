//! anastasia — headed by default; `anastasia headless` runs the engine alone. Both start
//! local-only without credentials. `anastasia login` and `anastasia logout` select the
...
enum Command {
    ...
    Sync,
    /// Manage `anastasia headless` as a background service (launchd / systemd --user).
    Daemon {
        #[command(subcommand)]
...
#[derive(Subcommand)]
enum DaemonCommand {
    /// Install, enable, and start the service (captures ANASTASIA_* env).
    Install,
...
/// Local/self-hosted edge. Anastasia ships without a production sync service.
const DEFAULT_EDGE_URL: &str = "http://127.0.0.1:8787";

fn edge_url_from_env() -> String {
    std::env::var("ANASTASIA_EDGE_URL")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| DEFAULT_EDGE_URL.into())
}

/// WorkOS is opt-in; Anastasia never inherits another product's credentials.
fn workos_client_id_from_env(edge_token: &Option<String>) -> Option<String> {
    match std::env::var("ANASTASIA_WORKOS_CLIENT_ID") {
    ...
    match cli.command {
        Some(Command::Headless) => {
            ...
            runtime.block_on(async {
                let engine = anastasia_engine::Engine::new(engine_config_from_env());
                engine.run().await
            })
        }
        Some(Command::Login) => {
        ...
        Some(Command::Update { check }) => {
            ...
            runtime.block_on(update_cli::update(
                &anastasia_update::release_api_url(),
                check,
            ))
        }
        ...
        None => {
            let edge_token = std::env::var("ANASTASIA_EDGE_TOKEN").ok();
            // Headed: the UI probes ANASTASIA_IPC_PORT and connects to a running
            // daemon, or embeds the engine in-process (ARCHITECTURE §1).
            anastasia_ui::run_app(anastasia_ui::UiConfig {
                data_dir: std::env::var_os("ANASTASIA_DATA_DIR")
                    .map(std::path::PathBuf::from)
                    .unwrap_or_else(dirs_data_dir),
                ipc_port: std::env::var("ANASTASIA_IPC_PORT")
                    .ok()
                ...
                edge_token,
                org_id: std::env::var("ANASTASIA_ORG_ID").ok(),
                default_harness: anastasia_ui::HarnessId::ClaudeCode,
            });
            Ok(())
        }
    }
...
fn engine_config_from_env() -> anastasia_engine::EngineConfig {
    // Dev-mode bearer (no WorkOS): an explicit token enables sync.
    let edge_token = std::env::var("ANASTASIA_EDGE_TOKEN").ok();
    anastasia_engine::EngineConfig {
        data_dir: std::env::var_os("ANASTASIA_DATA_DIR")
            .map(std::path::PathBuf::from)
            .unwrap_or_else(dirs_data_dir),
        edge_url: edge_url_from_env(),
        ...
        // WorkOS mode: the signed-in session's org wins; ANASTASIA_ORG_ID (dev
...
async fn sync_cli(ipc_port: u16) -> anyhow::Result<()> {
    let client = anastasia_rpc::connect_ws(&format!("ws://127.0.0.1:{ipc_port}"))
        .await
        .map_err(|e| {
            anyhow::anyhow!(
                "no engine listening on 127.0.0.1:{ipc_port} ({e}) — is anastasia running?"
            )
        })?;
    let status = client
        .call(anastasia_rpc::methods::SYNC_STATUS, serde_json::json!({}))
        .await
...
/// `{data_dir}/logs/anastasia-{mode}.log`, previous launch preserved as `.old`.
...
/// locked logs to `anastasia-{mode}.{pid}.log` instead; the next lock-holding
/// launch sweeps pid-suffixed files older than a week.
fn open_log_file(mode: &str) -> Option<std::fs::File> {
    let dir = std::env::var_os("ANASTASIA_DATA_DIR")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(dirs_data_dir)
        .join("logs");
...
fn open_log_file_in(dir: &std::path::Path, mode: &str) -> Option<std::fs::File> {
    ...
    let path = dir.join(format!("anastasia-{mode}.log"));
    #[cfg(unix)]
    {
        ...
        if rc != 0 {
            ...
            return std::fs::File::create(
                dir.join(format!("anastasia-{mode}.{}.log", std::process::id())),
            )
            .ok();
        }
        ...
        drop(existing);
        if preexisting {
            let _ = std::fs::rename(&path, dir.join(format!("anastasia-{mode}.log.old")));
        }
...
mod log_file_tests {
    ...
    fn second_launch_never_rotates_a_live_processes_log() {
        ...
        assert!(dir.join("anastasia-headed.log").is_file());
        ...
        let pid_path = dir.join(format!("anastasia-headed.{}.log", std::process::id()));
        assert!(pid_path.is_file(), "expected pid-suffixed overflow log");
        assert!(
            !dir.join("anastasia-headed.log.old").exists(),
        ...
        assert!(
            dir.join("anastasia-headed.log.old").is_file(),
            "rotation resumes"
        );
        drop(third);
    }
...
/// Delete `anastasia-{mode}.{pid}.log` overflow files older than a week — they
...
fn sweep_stale_pid_logs(dir: &std::path::Path, mode: &str) {
    ...
    let prefix = format!("anastasia-{mode}.");
