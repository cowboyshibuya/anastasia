//! `anastasia daemon …` — install/manage `anastasia headless` as a background service:
...
//! `ANASTASIA_*` environment captured at install time, so
//! `ANASTASIA_EDGE_URL=… anastasia daemon install` bakes that override in.
//!
//! Auth is decoupled: without a saved session the service remains up on the
//! local-only profile. `anastasia login` and a service restart opt into sync.
...
const LAUNCHD_LABEL: &str = "app.anastasia.daemon";
/// Same unit name the curl|sh installer (`edge/src/install.sh`) writes, so
/// `anastasia daemon …` manages that installation rather than a competing copy.
const SYSTEMD_UNIT: &str = "anastasia.service";
...
/// default PATH won't find); the `ANASTASIA_*`/logging vars only when set.
const CAPTURED_ENV: &[&str] = &[
    "PATH",
    "ANASTASIA_DATA_DIR",
    "ANASTASIA_EDGE_URL",
    "ANASTASIA_EDGE_TOKEN",
    "ANASTASIA_ORG_ID",
    "ANASTASIA_WORKOS_CLIENT_ID",
    "ANASTASIA_WORKOS_API_BASE",
    "ANASTASIA_IPC_PORT",
    "ANASTASIA_CALLBACK_PORT",
    "ANASTASIA_HARNESS",
    "ANASTASIA_DEVICE_NAME",
    "RUST_LOG",
];

pub fn install(data_dir: &Path) -> anyhow::Result<()> {
    let exe = std::env::current_exe().context("resolving the anastasia executable path")?;
    ...
    if cfg!(target_os = "macos") {
        ...
        bail!("anastasia daemon is only supported on macOS (launchd) and Linux (systemd)");
    }
    println!(
...
pub fn start() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        let plist = launchd_plist_path()?;
        if !plist.exists() {
            bail!("not installed — run `anastasia daemon install` first");
        }
        ...
        if !output.status.success() {
            println!(
                ...
                if launchd_plist_path()?.exists() {
                    " (installed — `anastasia daemon start`)"
                } else {
                    " (not installed — `anastasia daemon install`)"
                }
            );
            return Ok(());
        }
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=Anastasia headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
...
/// The ExecStart binary path. An exe under `~/.anastasia/app/` came from the
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".anastasia/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.anastasia/app/current/anastasia".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn systemd_unit_shape() {
        let unit = render_systemd_unit(
            Path::new("/usr/local/bin/anastasia"),
            &[
                ("PATH".into(), "/usr/bin:/bin".into()),
                ("ANASTASIA_EDGE_URL".into(), "https://edge.example".into()),
                ("RUST_LOG".into(), "info,anastasia=\"debug\"".into()),
            ],
        );
        assert!(unit.contains("ExecStart=/usr/local/bin/anastasia headless\n"));
        assert!(unit.contains("Environment=\"PATH=/usr/bin:/bin\"\n"));
        assert!(unit.contains("Environment=\"ANASTASIA_EDGE_URL=https://edge.example\"\n"));
        // Inner quotes escaped so systemd re-parses the value verbatim.
        assert!(unit.contains("Environment=\"RUST_LOG=info,anastasia=\\\"debug\\\"\"\n"));
        ...
        assert!(unit.contains("EnvironmentFile=-%h/.anastasia/env"));
    ...
    fn curl_installer_always_starts_the_local_capable_service() {
        ...
        assert!(installer.contains("systemctl --user enable anastasia"));
        assert!(installer.contains("systemctl --user restart anastasia"));
    }
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.anastasia/app/0.3.0/anastasia"),
                Some(Path::new("/home/u")),
            ),
            "%h/.anastasia/app/current/anastasia"
        );
        // Source build: literal path.
        assert_eq!(
            exec_path_for(
                Path::new("/src/target/debug/anastasia"),
                Some(Path::new("/home/u"))
            ),
            "/src/target/debug/anastasia"
        );
    }
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            Path::new("/Users/x/anastasia & co/anastasia"),
            &[("ANASTASIA_EDGE_URL".into(), "https://e?a=1&b=2".into())],
            Path::new("/Users/x/.anastasia/daemon.log"),
        );
        assert!(plist.contains("<key>Label</key><string>app.anastasia.daemon</string>"));
        // XML-escaped exe path and env value.
        assert!(plist.contains("<string>/Users/x/anastasia &amp; co/anastasia</string>"));
        ...
        assert!(
            plist.contains(
                "<key>StandardOutPath</key><string>/Users/x/.anastasia/daemon.log</string>"
            )
        );
    }
}
