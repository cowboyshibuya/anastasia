...
A native SwiftUI viewport onto the anastasia mesh. The phone is a **peer
...
```sh
cd apps/ios
xcodebuild -project Anastasia.xcodeproj -scheme Anastasia \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' build
```

Or open `Anastasia.xcodeproj` in Xcode and run. Dependencies (SPM, resolved
