...
final class AppModel {
    ...
    @ObservationIgnored @AppStorage("edgeURL") var edgeURLString = "http://127.0.0.1:8787"
    @ObservationIgnored @AppStorage("authMode") var authModeRaw = AppConfig.Mode.dev.rawValue
    ...
    func restore() {
        ...
        let mode = AppConfig.Mode(rawValue: authModeRaw) ?? .dev
        switch mode {
        case .dev:
    ...
    private func startPathMonitor() {
        ...
        monitor.start(queue: DispatchQueue(label: "anastasia.path-monitor"))
        pathMonitor = monitor
    }
