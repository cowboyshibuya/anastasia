...
// The anastasia mark on black, one white button — the old mobile app's Gate.
//
// Hosted authentication is intentionally unconfigured until Anastasia owns
// its production edge and WorkOS client.

import AuthenticationServices
import SwiftUI

enum Endpoints {
    static let edgeURL = URL(string: "http://127.0.0.1:8787")!
    static let workosClientId = ""
    static let workosAPIBase = "https://api.workos.com"
    static let callbackScheme = "anastasia"
...
struct SignInView: View {
    ...
    /// The AuthKit code flow: system browser session → anastasia://callback with
...
/// The actual anastasia mark — the desktop's 34-cell logo
/// (crates/ui/assets/icons/anastasia-logo.svg), cells scaled from its 820×940
/// viewbox and tinted by `color`.
struct AnastasiaMark: View {
...
struct AnastasiaMarkShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for (x, y) in AnastasiaMark.cells {
