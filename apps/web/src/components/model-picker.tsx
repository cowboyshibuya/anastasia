import type { AgentSession, ProviderKind, ProviderModel, ProviderProbe } from '@anastasia/client'
import { Popover } from '@base-ui/react/popover'
import { useEffect, useRef, useState, type RefObject } from 'react'
import { ProviderIcon, PROVIDERS, providerMeta, AnastasiaIcon } from '@/components/anastasia-icon'
        ...
        >
          ...
          <div className="flex h-full w-[50px] shrink-0 flex-col items-center gap-1 overflow-y-auto border-r bg-background p-[5px]">
            <ModelTab active={tab === 'favorites' && !query} label={t('models.favorites')} onClick={() => { setTab('favorites'); setQuery(''); setHighlight(null) }}>
              <AnastasiaIcon className="size-[17px]" name="star" />
            </ModelTab>
              ...
              {rows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <AnastasiaIcon className={cn('size-3.5 text-[var(--text-ghost)]', favorite && 'text-amber-500')} name={favorite ? 'starFilled' : 'star'} />
                    </span>
                  </div>
                )
              })}
