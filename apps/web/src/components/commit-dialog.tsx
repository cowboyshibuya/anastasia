...
import type { AgentSession, Project } from '@anastasia/client'
import { useEffect, useState, type RefObject } from 'react'
import { toast } from 'sonner'
import { Dialog, DialogContent, DialogTitle } from '@/components/ui/dialog'
import { AnastasiaIcon, type AnastasiaIconName } from '@/components/anastasia-icon'
import {
  commitWorkspace,
  daemonKeys,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[420px] overflow-hidden rounded-[18px] bg-[var(--raised)] p-0" finalFocus={returnFocus}>
        ...
        <div className="flex h-12 items-center gap-2.5 px-4 text-[14px]">
          <AnastasiaIcon className="size-[15px]" name="gitBranch" />
        ...
        <div className="px-2">
          ...
          >
            <span className={cn(
              ...
              {includeUnstaged && <AnastasiaIcon className="size-3" name="check" />}
            </span>
...
function CommitActionRow({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <AnastasiaIcon className={cn('size-3.5 text-[var(--text-secondary)]', pending && 'motion-safe:animate-spin')} name={pending ? 'loaderCircle' : icon} />
