              ...
              {(item.key.kind !== 'subagent' || stoppable) && (
                <span className="relative grid size-6 shrink-0 place-items-center">
                  ...
                  {stoppable && (
                    ...
                    >
                      <AnastasiaIcon className="size-3 text-[var(--text-tertiary)]" name="stopFilled" />
                    </button>
                  )}
                </span>
              )}
...
function EnvironmentAction({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <AnastasiaIcon className="size-3.5 text-[var(--text-secondary)]" name={icon} />
      <span className="min-w-0 flex-1 truncate text-left">{label}</span>
      {trailing && <AnastasiaIcon className="size-[13px] text-[var(--text-tertiary)]" name={trailing} />}
    </button>
  )
}
  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center px-8 pb-12">
      <div className="max-w-sm text-center">
        <AnastasiaIcon className="mx-auto size-6 text-ring" name="sparkle" />
        ...
        <div className="mt-5 flex flex-col items-center gap-2">
          <Button className="rounded-full" onClick={onAddProject}>
            <AnastasiaIcon name="plus" /> {t('sidebar.add_project')}
          </Button>
          <Button
